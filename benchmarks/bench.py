...
def calculate_metrics(
    ...
    dur_s: float,
    tokenizer: PreTrainedTokenizerBase,
) -> Tuple[BenchmarkMetrics, Dict[str, List]]:
    actual_output_lens = []
    total_input = 0
    completed = 0
    tpots = []
    ...
    for i in range(len(outputs)):
        ...
        if outputs[i].success:
            ...
            tpot = None
            if output_len > 1:
                tpot_val = (outputs[i].latency - outputs[i].ttft) / (output_len - 1)
                tpots.append(tpot_val)
                tpot = tpot_val * 1000
            
            ttfts.append(outputs[i].ttft)
            completed += 1
            latencies.append(outputs[i].latency)
            nlatencies.append(outputs[i].latency / output_len if output_len > 0 else 0)
            ...
            request_details.update({
                ...
                "tpot_ms": tpot,
                "latency_ms": outputs[i].latency * 1000,
                "error": None,
            })
        else:
        ...
        per_request_details.append(request_details)
...
async def benchmark(
    ...
    metrics, detailed_metrics = calculate_metrics(
    ...
    ttfts = detailed_metrics["ttfts"]
    real_tpots = detailed_metrics["tpots"]
    latencies = detailed_metrics["latencies"]
    nlatencies = detailed_metrics["nlatencies"]
    actual_output_lens = detailed_metrics["actual_output_lens"]
    input_lens = detailed_metrics["input_lens"]
    est_lens = detailed_metrics["est_lens"]
    pred_scores = detailed_metrics["pred_scores"]
    aux_model_scores = detailed_metrics["aux_model_scores"]
    texts = detailed_metrics["texts"]
    per_request_details = detailed_metrics["per_request_details"]
    ...
    print("{s:{c}^{n}}".format(s='Time Normalized Latency',
                              n=50,
                              c='-'))
    print("{:<40} {:<10.2f}".format("Mean Nlatency (ms):", np.mean(nlatencies or 0) * 1000))
    print("{:<40} {:<10.2f}".format("Median Nlatency (ms):",
                                     np.median(nlatencies or 0) * 1000))
    print("{:<40} {:<10.2f}".format("P80 Nlatency (ms):", np.percentile(nlatencies or 0, 80) * 1000))
    print("{:<40} {:<10.2f}".format("P90 Nlatency (ms):", np.percentile(nlatencies or 0, 90) * 1000))
    print("{:<40} {:<10.2f}".format("P95 Nlatency (ms):", np.percentile(nlatencies or 0, 95) * 1000))
    print("{:<40} {:<10.2f}".format("P99 Nlatency (ms):", np.percentile(nlatencies or 0, 99) * 1000))
    print("=" * 50)

    if est_lens and est_lens[0] is not None:
        tau, p = scipy.stats.kendalltau(est_lens, actual_output_lens)
        print(f"Est Kendall's Tau: {tau} {p}")
    if pred_scores and pred_scores[0] is not None:
        tau, p = scipy.stats.kendalltau(pred_scores, actual_output_lens)
        print(f"Pred Kendall's Tau: {tau} {p}")
    if aux_model_scores and aux_model_scores[0] is not None:
    ...
    if eval_max_tpot:
        ...
        for i in range(len(outputs)):
            if outputs[i].success:
                max_tpot = max(outputs[i].ttft, max(outputs[i].itl) if outputs[i].itl else 0)
                max_tpots.append(max_tpot)
        print("{s:{c}^{n}}".format(s='MAX-TPOT',
                                      n=50,
                                      c='-'))
        print("{:<40} {:<10.2f}".format("Mean MAX-TPOT (ms):", np.mean(max_tpots or 0) * 1000))
        print("{:<40} {:<10.2f}".format("Median MAX-TPOT (ms):",
                                          np.median(max_tpots or 0) * 1000))
        print("{:<40} {:<10.2f}".format("P80 MAX-TPOT (ms):", np.percentile(max_tpots or 0, 80) * 1000))
        print("{:<40} {:<10.2f}".format("P90 MAX-TPOT (ms):", np.percentile(max_tpots or 0, 90) * 1000))
        print("{:<40} {:<10.2f}".format("P95 MAX-TPOT (ms):", np.percentile(max_tpots or 0, 95) * 1000))
        print("{:<40} {:<10.2f}".format("P99 MAX-TPOT (ms):", np.percentile(max_tpots or 0, 99) * 1000))
        print("=" * 50)  
...
def main(args: argparse.Namespace):
    ...
    # Save per-request metrics if requested
    if args.dump_requests_file:
        per_request_metrics = benchmark_result.pop("per_request_details", None)
        if per_request_metrics:
            with open(args.dump_requests_file, "w") as f:
                json.dump(per_request_metrics, f, indent=4)
            print(f"Dumped per-request metrics to {args.dump_requests_file}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a file to dump detailed metrics for each request.",
    )
